...
//        if (!isset($form[$field_id]['#options'][$selected])) {
		if (isset($selected) && !empty($form[$field_id]['#options'][$selected])) {
...
//        $option = (string) $option;
		if ($option instanceof TranslatableMarkup) {
			$option = $option->__toString();
		}
		elseif (is_object($option) && !empty($option->option) && is_string($string = reset($option->option))) {
			foreach ($option->option as $tid => $term) {
				$taxonomy = TRUE;
				$option = $string;
			}
		}
